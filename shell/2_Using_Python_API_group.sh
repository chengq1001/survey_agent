...
BASE_DIR="/mnt/bn/chenguoqing-lf/code/survey_agent/output/Hallucination+Reasoning"
...
for term1 in "${TERM_GROUPS_1[@]}"; do
    for term2 in "${TERM_GROUPS_2[@]}"; do
        ...
        combo_output_file="${BASE_DIR}/${combo_name}/${date_str}/survey_${TIMESTAMP}.md"
        combo_log_file="${BASE_DIR}/${combo_name}/${date_str}/survey_${TIMESTAMP}_log.txt"
        
        echo -e "\n--- 处理组合: $term1 + $term2 ---"
        echo "组合名称: $combo_name"
        echo "输出文件: $combo_output_file"
        echo "日志文件: $combo_log_file"
        
        # 创建日志目录
        mkdir -p "$(dirname "$combo_log_file")"
